...
class ServerControl {
	...
	synchronized int userExistsAndID(String uuid) {
		for (UserDescription u : users) {
			if (u.uuid.equals(uuid)) {
				return u.id;
			}
	...
	String sendMessage(int src, int dst, String msg, String receipt, String sign) {
		int nr = 0;
		String result;
		String path = null;
		...
		try {
			...
			saveOnFile(path + src + "_" + nr, "{\"msg\":\""+msg+"\",\"sign\":\""+sign+"\"}");
	...
	String recvMessage(int id, String msg) {
		...
		int senderid = Integer.parseInt(m.group(1));
		...
		try {
			//check if sender's signature was made by sender's private cc key
			String content = readMsgFile(id, msg);
			
			JsonObject jsoncontent = new JsonParser().parse(content).getAsJsonObject();
			String sign = jsoncontent.get("sign").getAsString();
			String jmsg = jsoncontent.get("msg").getAsString();
			result += "\""+jmsg+"\",\""+sign+"\"";
	...
	void storeReceipt(int id, String msg, String receipt) {
		...
		String path = userReceiptBox(Integer.parseInt(m.group(1))) + "/_" + id + "_" + m.group(2) + "_"
